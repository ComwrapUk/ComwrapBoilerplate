/**
 * Credits: https://github.com/adobe/franklin-sidekick-library/blob/main/src/plugins/blocks/blocks.js
 * Added parts tagged with [CUSTOM]
 */
...
export async function onBlockListCopyButtonClicked(context, event, container) {
  const {
...
export function loadBlock(context, event, container) {
  ...
  // [CUSTOM] Disable details view and content editable when isDocumentation
  if (context.isDocumentation) {
    sectionLibraryMetadata.contenteditable = 'false';
    sectionLibraryMetadata.hidedetailsview = true;
    sectionLibraryMetadata.disablecopy = true;
  }
...
export function loadTemplate(context, event, container) {
  const content = container.querySelector('.block-library');
  const {
    blockWrapper,
    blockData,
  ...
  // [CUSTOM] Set split-view primary size like for blocks
  const splitView = container.querySelector('.content sp-split-view');
  splitView.primarySize = '75%';
...
/**
 * Default Blocks decorate.
 * @param {*} container The container to render the plugin in
 * @param {*} data The data contained in the plugin sheet
 * @param {*} searchTerm unused
 * @param {*} context
 */
export default async function decorate(container, data, searchTerm, context) {
